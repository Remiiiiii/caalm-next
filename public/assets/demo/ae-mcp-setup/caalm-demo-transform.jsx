...
(function(){
  try {
    var f = new File("C:/Users/victo/Documents/ae-mcp-bridge/ae_transform_boot.txt");
    f.open("w"); f.write("boot " + new Date().toString()); f.close();
  } catch(e){}
})();
/**
...
function transformCaalmDemo(args) {
    args = args || {};
    var ASSET_DIR = "c:/Users/victo/Development/caalm-next/public/assets/icons/demo-video/generated/";
    var DASHBOARD_FILES = [
    ...
    function pickPoppinsLocal(currentFont) {
        ...
        if (lower.indexOf("extrabold") !== -1 || lower.indexOf("extra bold") !== -1) return "Poppins-ExtraBold";
        if (lower.indexOf("bold") !== -1) return "Poppins-Bold";
        if (lower.indexOf("semibold") !== -1 || lower.indexOf("semi bold") !== -1) return "Poppins-SemiBold";
    ...
    function isDashboardName(name) {
        ...
        return /screen|dashboard|mockup|ui|preview|browser|laptop|phone|monitor|interface|saas|hero|content|placeholder|plate|image|footage|shot/.test(n) || /demo-\d+/.test(n);
    }
    ...
    function collectTextComps() {
        ...
        comps.sort(function (a, b) { return a.num - b.num; });
        return comps;
    }
    ...
    function importDashboards(folderName) {
        ...
        for (var f = 0; f < DASHBOARD_FILES.length; f++) {
            ...
            if (!file.exists) {
                imported.push({ file: DASHBOARD_FILES[f], error: "missing", path: path });
                continue;
            }
            var existing = null;
            ...
            if (existing) {
                imported.push({ file: DASHBOARD_FILES[f], item: existing, reused: true });
                continue;
            }
    ...
    function findReplaceableLayers(comp, depth) {
        ...
        for (var i = 1; i <= comp.numLayers; i++) {
            ...
            var source = null;
            try { source = layer.source; } catch (e) { source = null; }
            if (source && isImageFootage(source)) {
                found.push({ layer: layer, comp: comp, score: isDashboardName(layer.name) ? 10 : 5 });
                ...
                if (isDashboardName(layer.name) || isDashboardName(source.name)) {
                    ...
                    if (nested.length) {
                        ...
                        found.push({ layer: layer, comp: comp, score: 8, replacePrecomp: true });
                    }
                } else if (depth === 0) {
    ...
    function addDashboardLayer(comp, footage) {
        ...
        try {
            ...
            if (sw > 0 && sh > 0) {
                ...
                layer.property("Position").setValue([comp.width * 0.58, comp.height * 0.52]);
            }
        } catch (e) {}
    ...
    function remapLayerColors(comp) {
        for (var i = 1; i <= comp.numLayers; i++) {
            ...
            try {
                ...
                if (src instanceof FootageItem && src.mainSource instanceof SolidSource) {
                    var color = src.mainSource.color;
                    var next = null;
                    if (lname === "bg" || lname.indexOf("background") !== -1) next = COLOR_BG;
            ...
            try {
                if (layer.matchName === "ADBE Vector Layer" || (layer.property && layer.property("Contents"))) {
                    var contents = layer.property("Contents");
                    if (contents) {
                        for (var c = 1; c <= contents.numProperties; c++) {
                            var group = contents.property(c);
                            try {
                                var fill = group.property("Contents") ? group.property("Contents").property("Fill") : null;
                                if (!fill) fill = group.property("Fill");
                                if (fill && fill.property("Color")) {
                                    var fillColor = fill.property("Color").value;
                                    var mapped = null;
                                    if (lname === "bg" || isDarkColor(fillColor)) mapped = COLOR_BG;
    ...
    function updateTextInComp(comp) {
        for (var i = 1; i <= comp.numLayers; i++) {
            ...
            try {
                ...
                report.fontFailures.push({ comp: comp.name, layer: layer.name, error: String(e) });
            }
        }
    }
    ...
    function discoverSample(comp) {
        ...
        for (var i = 1; i <= Math.min(comp.numLayers, 40); i++) {
            var layer = comp.layer(i);
            var info = { index: layer.index, name: layer.name, hasSource: false, sourceType: null, sourceName: null };
            try {
                if (layer.source) {
                    info.hasSource = true;
                    ...
                    if (layer.source instanceof CompItem) info.sourceType = "Comp";
                        ...
                        if (layer.source.mainSource instanceof SolidSource) info.sourceType = "Solid";
                        else if (layer.source.mainSource instanceof FileSource) info.sourceType = "File";
                        else info.sourceType = "Footage";
                    }
                }
            } catch (e) {}
            if (layer instanceof TextLayer) info.sourceType = info.sourceType || "Text";
            layers.push(info);
        }
    ...
    try {
        ...
        for (var si = 1; si <= app.project.numItems; si++) {
            ...
            if (sitem instanceof CompItem && /^Scene\s+\d+/i.test(sitem.name)) scenes.push(sitem);
        }
        ...
        if (!footageItems.length) throw new Error("No dashboard PNGs could be imported. Check asset paths.");
        ...
        for (var sc = 0; sc < scenes.length; sc++) {
            ...
            if (sceneCandidates.length) {
                if (replaceLayerSource(sceneCandidates[0].layer, footageItems[sc % footageItems.length])) {
        ...
        var saveFile = new File(Folder.myDocuments.fsName + "/" + saveName + ".aep");
        app.project.save(saveFile);
        report.savedTo = saveFile.fsName;
        report.message = "Transformed to CAALM light mode; cycled dashboards across Text comps; saved project.";
        app.endUndoGroup();
...
try {
  var outFile = new File(Folder.myDocuments.fsName + "/ae-mcp-bridge/ae_mcp_result.json");
  outFile.encoding = "UTF-8";
  outFile.open("w");
  outFile.write(__result);
  outFile.close();
  var cmdFile = new File(Folder.myDocuments.fsName + "/ae-mcp-bridge/ae_command.json");
  cmdFile.encoding = "UTF-8";
  cmdFile.open("w");
  cmdFile.write('{"command":"transformCaalmDemo","status":"completed","timestamp":"' + new Date().toISOString() + '"}');
  cmdFile.close();
} catch (e) {}

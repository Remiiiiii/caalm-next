(() => {
	...
	for (var i = 1; i <= app.project.numItems; i++) {
		...
		if (item instanceof FootageItem && /^demo-0/.test(item.name))
	...
	for (var c = 1; c <= app.project.numItems; c++) {
		if (
			...
			app.project.item(c).name === "CAALM Presentation - Color"
		) {
	...
	if (color) {
		for (var b = 1; b <= color.numLayers; b++) {
			...
			if (
				ly.name === "BG" &&
				ly.source &&
				ly.source.mainSource &&
				ly.source.mainSource.color
			) {
	...
	var out = {
		textCompsWithDashboard: textWithDash,
		footageNames: footageNames,
		sampleTextComp: sample,
		bgColorSample: bgCheck,
	};
	var f = new File(

...
const ContractExpiryAlertsWidget = ({
  ...
  const { contracts, loading: isLoading, error } = useContracts();
  ...
  if (isLoading) {
    return (
      <Card className={`bg-white/30 backdrop-blur border border-white/40 shadow-lg ${className}`}>
        ...
        {showSettings && !isMinimized && (
          <div className="flex items-center space-x-4 mt-3 pt-3 border-t border-white/20">
            <div className="flex items-center space-x-2">
              ...
              <select
                value={filterDays}
                onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setFilterDays(Number(e.target.value))}
            ...
            <Button
              ...
              className={`text-xs h-6 px-2 ${showExpired ? 'bg-red-100 text-red-800' : 'text-slate-600'}`}
            >
              ...
              {filteredContracts.length > maxVisible && (
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/20">
                  ...
                  <span className="text-xs text-slate-600">
                    {currentIndex + 1}-{Math.min(currentIndex + maxVisible, filteredContracts.length)} of {filteredContracts.length}
                  </span>

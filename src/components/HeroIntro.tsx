...
function safePlay(video: HTMLVideoElement | null) {
	...
	if (result !== undefined) {
		result.catch(() => {
			// Autoplay policy, tab in background, or power-saving pause — safe to ignore
		});
	}
}
...
const HeroIntro = () => {
	...
	// Resume playback when the tab becomes visible again (browser may pause background video)
	useEffect(() => {
		if (!videoDuration || prefersReducedMotion) return;

		const onVisibility = () => {
			if (document.visibilityState !== "visible") return;
			safePlay(showA ? videoA.current : videoB.current);
		};
	...
	return (
		<section className="relative flex flex-col items-center justify-center pt-20 pb-8 bg-linear-to-b from-white to-blue-50 overflow-hidden">

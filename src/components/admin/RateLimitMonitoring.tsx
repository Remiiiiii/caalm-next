...
export default function RateLimitMonitoring() {
  const { metrics, isLoading, error, refresh, resetMetrics } = useRateLimitMetrics({
    enableRealTime: true,
  ...
  const handleReset = async () => {
    if (!confirm('Are you sure you want to reset all rate limit metrics? This action cannot be undone.')) {
      return;
    }
      ...
      <div className="glass-card w-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50">
          <div className="text-center text-destructive">
            ...
            <p>{error}</p>
            <Button onClick={refresh} variant="outline" className="mt-4 primary-btn px-3 sm:px-4">
    ...
    <div className="glass-card w-full overflow-hidden">
      ...
      <div className="sticky top-0 z-10 bg-gradient-to-r from-blue-50 to-indigo-50 py-4 border-b border-slate-200 mt-4">
        ...
        <div className="flex items-center justify-end gap-2 px-6 mt-4 pb-2">
          <Button onClick={refresh} variant="outline" size="sm" className="primary-btn px-3 sm:px-4">
            <RefreshCw className="h-4 w-4 mr-2" />
            Refresh
          </Button>
          <Button
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-slate-50 space-y-6">
        ...
        <Card className="glass-card hover:shadow-drop-3 transition-all duration-300">
          ...
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 pt-6">
            <CardTitle className="text-sm font-medium sidebar-gradient-text">Total Requests</CardTitle>
            <Activity className="h-4 w-4 text-[#0f5384]" />
          </CardHeader>
          <CardContent className="bg-slate-50">
            <div className="text-2xl font-bold text-slate-900">{summary.totalRequests.toLocaleString()}</div>
        ...
        <Card className="glass-card hover:shadow-drop-3 transition-all duration-300">
          <div className="glass-card-cap" />
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 pt-6">
            <CardTitle className="text-sm font-medium sidebar-gradient-text">Blocked Requests</CardTitle>
        ...
        <Card className="glass-card hover:shadow-drop-3 transition-all duration-300">
          <div className="glass-card-cap" />
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 pt-6">
            <CardTitle className="text-sm font-medium sidebar-gradient-text">Violations</CardTitle>
            <AlertTriangle className="h-4 w-4 text-[#0f5384]" />
          </CardHeader>
          <CardContent className="bg-slate-50">
            <div className="text-2xl font-bold text-slate-900">{summary.violations.toLocaleString()}</div>
        ...
        <Card className="glass-card hover:shadow-drop-3 transition-all duration-300">
          <div className="glass-card-cap" />
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 pt-6">
            <CardTitle className="text-sm font-medium sidebar-gradient-text">Efficiency</CardTitle>
            <TrendingUp className="h-4 w-4 text-[#0f5384]" />
          </CardHeader>
          <CardContent className="bg-slate-50">
            <div className="text-2xl font-bold text-green-600">{summary.efficiency}</div>
        ...
        <Card className="glass-card hover:shadow-drop-3 transition-all duration-300">
          <div className="glass-card-cap" />
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 pt-6">
            <CardTitle className="text-sm font-medium sidebar-gradient-text">Avg Latency</CardTitle>
            <Clock className="h-4 w-4 text-[#0f5384]" />
          </CardHeader>
          <CardContent className="bg-slate-50">
            <div className="text-2xl font-bold text-slate-900">{summary.averageLatency}</div>
            <p className="text-xs text-slate-600 mt-1">Rate limit check time</p>
          </CardContent>
        </Card>
      </div>
        ...
        {topViolators.length > 0 && (
          ...
          <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-slate-200 mt-4">
            <div className="flex items-center gap-3">
              ...
              <CardTitle className="text-xl font-semibold sidebar-gradient-text">Top Violators</CardTitle>
            </div>
          </CardHeader>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-50">
                    <TableHead className="text-slate-900 font-semibold">Identifier</TableHead>
                    <TableHead className="text-slate-900 font-semibold">Violations</TableHead>
        ...
        {Object.keys(endpointStats).length > 0 && (
          ...
          <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-slate-200 mt-4">
            <div className="flex items-center gap-3">
              ...
              <CardTitle className="text-xl font-semibold sidebar-gradient-text">Endpoint Statistics</CardTitle>
            </div>
          </CardHeader>
          <CardContent className="bg-slate-50 p-6">
                ...
                .map(([endpoint, stats]) => (
                  ...
                  >
                    <div className="flex-1">
                      <p className="font-medium text-sm text-slate-900">{endpoint}</p>
                      <p className="text-xs text-slate-600 mt-1">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-right">
                        <p className="text-sm font-semibold text-slate-900">
                          {stats.requests > 0
                            ? ((stats.violations / stats.requests) * 100).toFixed(1)
                            : 0}
                          %
                        </p>
                        <p className="text-xs text-slate-600">violation rate</p>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {!isLoading && summary.totalRequests === 0 && (
          ...
          <CardContent className="pt-6 bg-slate-50">
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-semibold mb-2 text-slate-900">No Rate Limit Data</h3>
              <p className="text-slate-600 mb-4">
                Rate limit metrics will appear here once API requests are made.
              </p>
              <Button onClick={refresh} variant="outline" className="primary-btn px-3 sm:px-4">
                <RefreshCw className="h-4 w-4 mr-2" />
                Refresh
              </Button>
            </div>

...
const CALENDAR_TOOLBAR_TITLE_CLASS = "text-lg font-bold sidebar-gradient-text";

/** In-card month label beside chevrons */
const CALENDAR_CARD_MONTH_TITLE_CLASS = "text-base font-medium text-slate-700";
...
const CALENDAR_TAB_ACTIVE = "border-blue/30 bg-blue/10 text-[#0f5384]";

const CALENDAR_TODAY_BADGE = "bg-blue-100 font-semibold text-[#2563eb]";
...
function contractExpiryToCalendarEvent(
	...
	const name = String(contract.contractName ?? contract.name ?? "Contract");
...
function licenseExpiryToCalendarEvent(
	...
	const date = parseCalendarDate(
		(license.licenseExpiryDate ?? license.expirationDate) as string | undefined,
	);
	if (!date) return null;
	...
	return (
		<div className="flex flex-wrap gap-x-4 gap-y-2 px-[18px] pb-3.5">
			<div className="flex items-center gap-1.5 text-[10.5px] text-slate-600">
				<span className="h-1.5 w-1.5 rounded-full bg-[#0f5384]" aria-hidden />
				Contract event
			</div>
			<div className="flex items-center gap-1.5 text-[10.5px] text-slate-600">
			...
			<div className="flex items-center gap-1.5 text-[10.5px] text-slate-600">
				<span className="h-1.5 w-1.5 rounded-full bg-[#a06ce2]" aria-hidden />
				Audit event
			</div>
			<div className="flex items-center gap-1.5 text-[10.5px] text-slate-600">
							...
							>
								<span
									className={cn(
										...
										!today && !selected && "group-hover:bg-slate-100",
									)}
								>
									{format(day, "d")}
								</span>
								{(hasContract || hasDeadline || hasLicense || hasAudit) && (
					...
					{weekDays.map((day) => (
						<div key={day.toISOString()} className="p-2">
							...
							<button
								...
								className={cn(
									"mx-auto flex h-[26px] w-[26px] cursor-pointer items-center justify-center rounded-full text-lg font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0f5384]/40",
									isToday(day)
		...
		<div className="flex h-full w-full min-w-0 flex-col gap-3.5">
			...
			>
				...
				{viewMode === "month" ? (
					...
					<div className="px-3.5 pb-4 sm:px-[14px]">{WeekView()}</div>
				)}

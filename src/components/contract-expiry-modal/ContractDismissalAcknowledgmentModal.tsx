...
interface ContractDismissalAcknowledgmentModalProps {
	isOpen: boolean;
	onClose: () => void;
	contract: UIFileDoc;
	onSuccess: () => void;
...
export default function ContractDismissalAcknowledgmentModal({
	isOpen,
	onClose,
	contract,
	onSuccess,
}: ContractDismissalAcknowledgmentModalProps) {
	...
	const handleSubmit = async () => {
		if (!signatureRef.current || signatureRef.current.isEmpty()) {
			toast({
				title: "Acknowledgment mark required",
				description: "Please draw your acknowledgment mark before submitting.",
				variant: "destructive",
			});
			return;
		}
		...
		if (!isConfirmed) {
			toast({
				...
				description: "Please confirm you reviewed this expiry alert.",
				variant: "destructive",
			});
			return;
		}
				...
				>
					...
					<div className="glass-dialog-wizard-header mt-4">
						...
						<p className="text-sm text-slate-600 mt-1 ml-14">
							This is an internal acknowledgment only. It is not a legal
							e-signature and does not execute the contract.
						</p>
					</div>

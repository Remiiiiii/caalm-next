...
import { IT_NAVIGATION, filterITNavigationByPermissions } from '@/constants/it-navigation';
                  ...
                  <li className="sidebar-section-header mb-0 lg:mb-1 font-bold text-lg lg:text-xl">
                    <span className="flex items-center gap-2">
                      {section.header === 'Dashboard' ? (
                        ...
                        <span className="text-[#03AFBF]">
                          <Image
                            src={section.items[0]?.icon || '/assets/icons/dashboard.svg'}
                            alt={section.header.toLowerCase()}
                            width={24}
                            height={24}
                            priority={section.header === 'Dashboard' || section.header === 'System Overview'}
                          />
                        </span>
                      )}
                  ...
                  <div className="relative ml-3">
                    <ul className="flex flex-col gap-1 relative z-10">
                      {section.items.map((item, index) => {
                        const isActive =
                          pathname === item.url ||
                          (pathname?.startsWith(`${item.url}/`) && item.url !== '/dashboard/it');

                        return (
                          <li key={`${section.header}-${item.name}-${index}`} className="relative flex items-center">

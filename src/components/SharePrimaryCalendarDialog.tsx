...
> = ({ open, onOpenChange, onShared }) => {
  ...
  useEffect(() => {
    ...
    const searchUsers = async () => {
      ...
      try {
        ...
        console.error('[CLIENT] SharePrimaryCalendarDialog] Error searching users:', error);
      } finally {
        setSearching(false);
      }
    };
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      console.error('[CLIENT] SharePrimaryCalendarDialog] Error sharing calendar:', error);
      toast({
        title: 'Error',
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[600px] p-0 max-h-[90vh] flex flex-col">
        <VisuallyHiddenPrimitive.Root>
          ...
          <DialogDescription>
            Share your primary calendar with others and control what they can see
          </DialogDescription>
        </VisuallyHiddenPrimitive.Root>
            ...
            <div className="flex items-center">
              <div>
                ...
                <p className="text-sm text-slate-600 mt-1 ml-7">
                  Share your primary calendar with team members and control their access level
                </p>
              </div>
            </div>
          </div>
                      ...
                      ([level, { label, description }]) => (
                        <SelectItem key={level} value={level}>
                          <div className="flex items-start gap-2 py-1">
                            {getPermissionIcon(level as CalendarPermissionLevel)}
                            <div>

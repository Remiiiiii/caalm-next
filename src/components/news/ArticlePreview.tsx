...
const ArticlePreview: React.FC<ArticlePreviewProps> = ({
  ...
  const getPriorityBadge = (priority: string) => {
    switch (priority) {
      case 'high':
        return <Badge className="bg-red/10 text-red border-red/20 text-xs">High</Badge>;
      case 'medium':
        return <Badge className="bg-yellow/10 text-yellow border-yellow/20 text-xs">Medium</Badge>;
      case 'low':
        return <Badge className="bg-green/10 text-green border-green/20 text-xs">Low</Badge>;
      default:
        return null;
    }
  };

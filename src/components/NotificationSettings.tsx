...
const NotificationSettings: React.FC<NotificationSettingsProps> = ({
	...
	const handleGlobalSettingChange = useCallback(
		(field: string, value: boolean | string | number) => {
	...
	useEffect(() => {
			...
			try {
				...
				if (result.submitted) {
					...
					if (result.data?.phone_number) {
						...
						if (phoneNumber.trim()) {
							const normalized1 = normalizePhoneNumber(dbPhoneNumber);
							const normalized2 = normalizePhoneNumber(phoneNumber);
		...
		checkFormStatus();
		return () => controller.abort();
	}, [open, user?.$id, handleGlobalSettingChange, phoneNumber]);
	...
	useEffect(() => {
		...
		globalSettings.smsNotifications, // Also disable SMS Notifications switch when Enable SMS is off

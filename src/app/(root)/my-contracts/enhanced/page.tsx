...
import { ArrowLeft, Building2, FileText, Search, Users } from "lucide-react";
...
import type { ContractDepartment } from "../../../../../constants";

const EnhancedMyContractsPage = () => {
	const { role, division, loading, error } = useUserRole();
	const { permissions } = usePermissions();
	const [_contracts, setContracts] = useState<UIFileDoc[]>([]);
	const [filteredContracts, setFilteredContracts] = useState<UIFileDoc[]>([]);
	const [_selectedDepartment, _setSelectedDepartment] =
		useState<ContractDepartment>("Operations");
	const [_selectedDivision, _setSelectedDivision] = useState<string>("");
	...
	const _handleSearchQuery = (query: string) => {

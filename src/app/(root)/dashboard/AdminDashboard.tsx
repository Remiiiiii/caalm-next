...
import { SelectScrollable, SelectItem } from '@/components/ui/select-scrollable';
import { useToast } from '@/hooks/use-toast';
import { useAdminStats } from '@/hooks/useAdminStats';
import { useUnifiedDashboardData } from '@/hooks/useUnifiedDashboardData';
import { useOrganization } from '@/contexts/OrganizationContext';
import { StatCardSkeleton, FileItemSkeleton, TableRowSkeleton } from '@/components/ui/skeletons';
...
const AdminDashboard = ({ user }: AdminDashboardProps) => {
  ...
  const widgetGap = 16;
  const pageWidth = widgetsPerPage * widgetWidth + (widgetsPerPage - 1) * widgetGap;

  const scrollToPage = (pageNumber: number) => {
    if (widgetScrollRef.current) {
      const scrollAmount = pageNumber * pageWidth;
      widgetScrollRef.current.scrollTo({ left: scrollAmount, behavior: 'smooth' });
  ...
  const [removingInvitations, setRemovingInvitations] = useState<Set<string>>(new Set());
  const [addingInvitations, setAddingInvitations] = useState<Set<string>>(new Set());
  ...
  const revokeInvitation = async (token: string) => {
    const response = await fetch(`/api/invitations/${token}/revoke`, { method: 'PUT' });
    if (!response.ok) throw new Error('Failed to revoke invitation');
    await refreshUnified();
  };

  const deleteInvitation = async (token: string) => {
    const response = await fetch(`/api/invitations/${token}/delete`, { method: 'DELETE' });
  ...
  const handleRefreshUsers = async () => {
    ...
    try {
      await refreshUnified();
      toast({ title: 'Success', description: 'User list refreshed successfully' });
    } catch (e) {
      toast({ title: 'Error', description: 'Failed to refresh user list', variant: 'destructive' });
    } finally {
  ...
  const handleInviteSubmit = async (e: React.FormEvent) => {
    ...
    if (!inviteForm.selectedUserId) {
      toast({ title: 'Error', description: 'Please select a user to invite', variant: 'destructive' });
      return;
    }

    setLoadingInvite(true);
    const selectedUser = (uninvitedUsers as UninvitedUser[]).find((u) => u.$id === inviteForm.selectedUserId);
    if (!selectedUser) {
    ...
    try {
      ...
      toast({ title: 'Invitation Sent', description: `Invitation sent to ${selectedUser.fullName} (${selectedUser.email})` });
      setInviteForm({ selectedUserId: '', role: '', department: '', division: '' });
      setTimeout(() => {
  ...
  const confirmRevoke = async () => {
    ...
    try {
      ...
      toast({ title: 'Invitation Revoked', description: 'The invitation has been successfully revoked.' });
    } catch {
      toast({ title: 'Error', description: 'Failed to revoke invitation. Please try again.', variant: 'destructive' });
    } finally {
      setShowRevokeDialog(false);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (currentData?.data?.invitations) {
        const updatedInvitations = (currentData.data.invitations as Invitation[]).filter((inv) => inv.token !== deleteToken);
        await refreshUnified({ ...currentData, data: { ...currentData.data, invitations: updatedInvitations } }, { revalidate: false });
      }
      toast({ title: 'Invitation Deleted', description: 'The invitation has been permanently deleted.' });
  ...
  const handleResend = async (token: string) => {
    ...
    try {
      const response = await fetch(`/api/invitations/${token}/resend`, { method: 'POST' });
      if (!response.ok) {
      ...
      toast({ title: 'Error', description: err instanceof Error ? err.message : 'Failed to resend invitation', variant: 'destructive' });
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-white/30 backdrop-blur border border-white/40 shadow-lg">
        <CardContent className="p-3">
          ...
          <div
            ...
            style={{ scrollbarWidth: 'none', msOverflowStyle: 'none', width: `${pageWidth}px`, maxWidth: `${pageWidth}px` }}
          >
            ...
            <div className="flex gap-2 min-w-full -ml-2">
              ...
              <ContractExpiryAlertsWidget maxVisible={2} showSettings={false} compact={true} />
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid lg:grid-cols-6 gap-6">
        ...
        <Card className="bg-white/30 backdrop-blur border border-white/40 shadow-lg lg:col-span-3">
          <CardContent className="p-4">
            <CalendarView
              user={user as any}
              onEventCreate={() => toast({ title: 'Success', description: 'Event created successfully!' })}
            />
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="bg-white/30 backdrop-blur border border-white/40 shadow-lg">
        ...
        <CardContent>
          {unifiedLoading ? (
            <div className="space-y-4">{[1, 2, 3].map((i) => (<FileItemSkeleton key={i} />))}</div>
          ) : files && (files as any[]).length > 0 ? (
            <div className="max-h-[400px] overflow-y-auto pr-2 space-y-4">
              {(files as any[]).slice(0, 10).map((file: any) => {
                const fileDoc = file as unknown as FileDocument;
                return (
                  <div key={fileDoc.$id} className="border border-border rounded-lg p-4">
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        <Thumbnail type={fileDoc.type} extension={fileDoc.extension} url={fileDoc.url} />
                        <div className="flex flex-col gap-1 min-w-0 flex-1">
                          <h4 className="font-medium text-navy truncate max-w-[200px]">{fileDoc.name}</h4>
                          <p className="text-sm text-slate-dark">
                            <FormattedDateTime date={fileDoc.$createdAt as unknown as string} className="text-xs text-slate-light" />
                          </p>
                        </div>
                      </div>
              ...
              {(files as any[]).length > 10 && (
                <div className="text-center py-2">
                  <p className="text-xs text-slate-light">+{(files as any[]).length - 10} more files</p>
                </div>
              )}
            </div>
          ) : (
            <p className="text-center text-slate-light">No files uploaded</p>
          )}
        </CardContent>
      </Card>
          ...
          <form className="flex flex-col gap-4" onSubmit={handleInviteSubmit}>
            <div className="flex flex-row gap-2 items-center justify-between">
              <SelectScrollable
                ...
                onValueChange={(value) => setInviteForm({ ...inviteForm, selectedUserId: value })}
              ...
              <Button type="button" onClick={handleRefreshUsers} disabled={refreshLoading} className="bg-white/30 backdrop-blur border border-white/40 shadow-md text-slate-700 hover:bg-white/40">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center">
              ...
              <SelectScrollable
                ...
                onValueChange={(value) => setInviteForm({ ...inviteForm, division: value })}
            ...
            <Button type="submit" disabled={loadingInvite || (uninvitedUsers as UninvitedUser[]).length === 0} className="bg-white/30 backdrop-blur border border-white/40 shadow-md text-slate-700 hover:bg-white/40">
              {loadingInvite ? 'Inviting...' : 'Send Invite'}
            </Button>
            {(uninvitedUsers as UninvitedUser[]).length === 0 && (
              <p className="text-sm text-gray-500 mt-2 text-center">No users found in Auth database</p>
            )}
          </form>
        </CardContent>
      </Card>
            ...
            <table className="min-w-full text-xs">
              <thead className="bg-gray-50 text-center">
                <tr>
                  ...
                  <th className="text-slate-700 text-center px-4 py-2">Email</th>
                  <th className="text-slate-700 text-center px-4 py-2">Role</th>
                  <th className="text-slate-700 text-center px-4 py-2">Invited</th>
                  <th className="text-slate-700 text-center px-4 py-2">Expires</th>
              ...
              <tbody>
                {unifiedLoading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="text-center py-8 text-gray-400">No pending invitations</td>
                  </tr>
                ) : (
                  (invitations as Invitation[]).map((inv) => (
                    ...
                    >
                      ...
                      <td>{inv.email}</td>
                      <td>{(inv.role || '').charAt(0).toUpperCase() + (inv.role || '').slice(1)}</td>
                      <td><ClientDate dateString={inv.$createdAt} /></td>
                      <td><ClientDate dateString={inv.expiresAt} /></td>
                      <td>
                      ...
                      <td className="space-x-2">
                        <Button
                          ...
                          className="bg-white/30 backdrop-blur border border-white/40 shadow-md text-slate-700"
                        >
                          {revokingToken === inv.token ? 'Revoking...' : 'Revoke'}
                        </Button>
                        <Button
                          size="sm"
                          variant="secondary"
                          ...
                          className="bg-white/30 backdrop-blur border border-white/40 shadow-md text-slate-700"
                        >
                          {resendingToken === inv.token ? 'Resending...' : 'Resend'}
                        </Button>
                        <Button
                          size="sm"
                        ...
                        >
                          {deletingToken === inv.token ? 'Deleting...' : <Trash2 className="h-4 w-4" />}
                        </Button>
                      </td>
                    </tr>
                  ))

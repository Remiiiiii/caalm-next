...
import { getUserPermissions, getUserRoles, getUserDefaultOrganization } from '@/lib/rbac/permissions';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const isSuperAdmin = userRoles.some((ur) => ur.roleId === 'role_super_admin');
    ...
    for (const rp of rolePermissions.rows) {
      ...
      if (!permissionExists || !roleExists) {
        issues.orphanedRolePermissions.push({
          ...
          issue: !permissionExists && !roleExists
    ...
    for (const rp of rolePermissions.rows) {
      rolePermissionCounts[rp.roleId] = (rolePermissionCounts[rp.roleId] || 0) + 1;
    }

	...
	try {
		...
		try {
			...
			if (Array.isArray(file.assignedManagers)) {
				const hasNulls = file.assignedManagers.some(
					(m: any) => m === null || m === undefined,
				);
				if (hasNulls) {
					const cleaned = file.assignedManagers.filter(
						(m: any) => m !== null && m !== undefined,
					);
					updateData.assignedManagers = cleaned.length > 0 ? cleaned : [];
					result.fixes.push(
						"Cleaned assignedManagers array (removed null values)",
					);
			...
			if (file.owner) {
				...
				result.steps.push(
					`Step 3: Clearing two-way relationship with user ${ownerId}...`,
				);
				...
				try {
					...
					try {
						...
						result.steps.push(
							`✓ Triggered user document update to refresh relationships`,
						);
					} catch (updateError: any) {
						result.steps.push(
							`⚠ Could not update user document: ${updateError.message}`,
						);
						// Continue anyway
					}
				} catch (error: any) {
				...
				result.fixes.push(
					"Cleared owner relationship (will auto-update user files relationship)",
				);
			...
			try {
				...
				const licensesWithFileRef = licenses.rows.filter(
					(l: any) =>
						l.fileRef === fileId ||
						(typeof l.fileRef === "object" && l.fileRef?.$id === fileId),
				);

				if (licensesWithFileRef.length > 0) {
					result.steps.push(
						`⚠ Found ${licensesWithFileRef.length} license(s) with fileRef`,
					);
					...
					for (const license of licensesWithFileRef) {
						try {
							...
							result.steps.push(
								`✓ Cleared fileRef from license ${license.$id}`,
							);
							result.fixes.push(
								`Cleared fileRef from license ${license.licenseName}`,
							);
						} catch (error: any) {
							result.errors.push(
								`Failed to clear fileRef from license ${license.$id}: ${error.message}`,
							);
							result.steps.push(
								`✗ Failed to clear fileRef from license ${license.$id}`,
							);
						}
					}
				} else {
			...
			if (appwriteConfig.contractsCollectionId) {
				...
				try {
					...
					if (contractsWithFileId.length > 0) {
						result.steps.push(
							`⚠ Found ${contractsWithFileId.length} contract(s) with fileId`,
						);
						...
						for (const contract of contractsWithFileId) {
							try {
								...
								result.steps.push(
									`✓ Cleared fileId from contract ${contract.$id}`,
								);
								result.fixes.push(
									`Cleared fileId from contract ${contract.contractName}`,
								);
							} catch (error: any) {
								result.errors.push(
								...
								result.steps.push(
									`✗ Failed to clear fileId from contract ${contract.$id}`,
								);
							}
						}
					} else {
			...
			result.steps.push(
				"Step 8: Waiting for relationship updates to propagate (3 seconds)...",
			);
			await new Promise((resolve) => setTimeout(resolve, 3000));

			// Step 9: Try to delete the file directly to get the exact error
			result.steps.push(
				"Step 9: Attempting direct deletion to capture error...",
			);
			try {

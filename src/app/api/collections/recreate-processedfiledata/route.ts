...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      result.errors.push(`Failed to delete processedFileData: ${error.message}`);
      throw error;
    }

    // Step 3: Create new processedFileData attribute with smaller size (10KB instead of 1MB)
    result.steps.push('Step 3: Creating new processedFileData attribute (10KB)...');
    try {
      ...
      result.errors.push(`Failed to create new processedFileData: ${error.message}`);
      throw error;
    }
    ...
    for (const [draftId, value] of Object.entries(backups)) {
      try {
        ...
        if (valueString.length <= 10240) {
          ...
          skipped++;
          result.steps.push(`⚠ Skipped draft ${draftId} - value too large (${valueString.length} bytes)`);
        }
      } catch (error: any) {
        result.errors.push(`Failed to restore draft ${draftId}: ${error.message}`);
      }
    }
    result.steps.push(`✓ Restored ${restored} draft(s), skipped ${skipped} draft(s)`);

    return NextResponse.json({
      success: true,
      message: 'Successfully recreated processedFileData attribute with smaller size',
      result,
    });
  } catch (error: any) {

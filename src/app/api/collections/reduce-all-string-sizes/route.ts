	...
	try {
		...
		for (const attr of attributesToReduce) {
			try {
				...
				result.steps.push(
					`✓ Backed up ${Object.keys(backups).length} draft(s)`,
				);
				...
				result.steps.push(
					`Step 3: Creating new ${attr.key} attribute (${attr.newSize} bytes)...`,
				);
				try {
					...
					result.steps.push(
						`✓ Created new ${attr.key} attribute (${attr.newSize} bytes)`,
					);
				} catch (error: any) {
					result.errors.push(
						`Failed to create new ${attr.key}: ${error.message}`,
					);
					throw error;
				}
				...
				for (const [draftId, value] of Object.entries(backups)) {
					try {
						...
						if (valueString.length <= attr.newSize) {
							...
							skipped++;
							result.steps.push(
								`⚠ Truncated ${attr.key} for draft ${draftId} (${valueString.length} -> ${truncated.length} bytes)`,
							);
						}
					} catch (error: any) {
						result.errors.push(
				...
				result.steps.push(
					`✓ Restored ${restored} draft(s), truncated ${skipped} draft(s)`,
				);

		...
		try {
			...
			if (draftId) {
				...
				const draftsResponse = await tablesDB.listRows({
					databaseId: appwriteConfig.databaseId,
					tableId: appwriteConfig.contractDraftsCollectionId,
					queries: [Query.greaterThanEqual("currentStep", 2), Query.limit(100)],
				});
				drafts = draftsResponse.rows;
				result.steps.push(`Found ${drafts.length} draft(s) to process`);
			}
			...
			for (const draft of drafts) {
				try {
					...
					if (!processedFileData?.name) {
						result.steps.push(
							`⚠ Draft ${draft.$id} has no processedFileData, skipping`,
						);
						continue;
					}
					...
					try {
						fileRow = await FileService.createOrUpdateFileRow(
							ownerId,
							accountId,
							{
					...
					if (draft.isCompleted || (draft.currentStep >= 2 && formData)) {
						result.steps.push(
							`Checking for contract in Contracts collection...`,
						);
						...
						const existingContracts = await tablesDB.listRows({
							...
							queries: [Query.equal("fileId", fileRow.$id), Query.limit(1)],
						});

						if (existingContracts.total > 0) {
							result.steps.push(
								`✓ Contract already exists: ${existingContracts.rows[0].$id}`,
							);
						} else {
							// Create contract row using ContractService
							result.steps.push(
								`Creating contract row in Contracts collection...`,
							);
						...
						result.steps.push(
							`⚠ Draft not completed yet, skipping contract creation`,
						);
					}
			...
			return errorResponse(
				error instanceof Error ? error : new Error("Failed to backfill drafts"),
				500,
				{ requestId },
			);
		}

...
export async function GET(request: NextRequest) {
	try {
		...
		allContracts.rows.forEach((doc) => {
			if (doc.contractName?.toLowerCase().includes(query.toLowerCase())) {
				suggestions.add(doc.contractName);
			}
			if (doc.vendor?.toLowerCase().includes(query.toLowerCase())) {
				suggestions.add(doc.vendor);
			}
			if (doc.contractType?.toLowerCase().includes(query.toLowerCase())) {
				suggestions.add(doc.contractType);
			}
			if (doc.department?.toLowerCase().includes(query.toLowerCase())) {

...
export async function POST(request: NextRequest) {
	try {
		...
		try {
			const existingType =
				await notificationService.getNotificationType("contract_dismissal");
			if (!existingType) {
				await notificationService.createNotificationType({
					...
					enabled: true,
					description:
						"Notifications when contracts are dismissed with electronic signature",
				});
				console.log("Created notification type: contract_dismissal");
			}
		} catch (typeError) {
			console.warn(
				"Could not create notification type (may already exist):",
				typeError,
			);
		}
		...
		try {
			...
			console.log(
				"Audit log entry created successfully for contract dismissal:",
				{
					eventId,
					contractId: body.contractId,
		...
		return NextResponse.json(
			{
				error:
					error instanceof Error ? error.message : "Failed to dismiss contract",
			},
			{ status: 500 },
		);
	}

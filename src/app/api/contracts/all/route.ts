...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const contractsToUpdate: Array<{ id: string; name: string; daysUntilExpiry: number }> = [];
    ...
    for (const contract of contractsResult?.rows || []) {
      ...
      const needsStatusUpdate = isExpired && (!contract.status || contract.status?.toLowerCase() !== 'expired');
      ...
      Promise.all(
        contractsToUpdate.map(async (contract) => {
          try {
            ...
            const expiryStr = contractsResult?.rows.find((c: any) => c.$id === contract.id)?.contractExpiryDate?.split('T')[0];
            if (expiryStr) {
              const [year, month, day] = expiryStr.split('-').map(Number);
              const expiryDate = new Date(year, month - 1, day);
              expiryDate.setHours(0, 0, 0, 0);
            ...
            console.log(`[API /contracts/all] Updated contract "${contract.name}" (${contract.id}) - daysUntilExpiry: ${contract.daysUntilExpiry}`);
          } catch (error) {
            console.error(`[API /contracts/all] Failed to update contract ${contract.id}:`, error);
          }
        })
      ).catch((error) => {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      if (activeContracts.length > 0) {
        console.log('[API /contracts/all] Contracts with status="active":', activeContracts);
      } else {
        console.log('[API /contracts/all] No contracts with status="active" found');
      }
    }

...
export type ConstituentOrgContext =
	| { ok: false; response: NextResponse }
	| { ok: true; user: AppUser; orgId: string };

/**
 * Resolve org from the request or the user's default org, then authorize.
 * Handlers must use this orgId — never a body-supplied tenant.
 */
export async function requireConstituentOrgContext(
	request: NextRequest,
	permission: PermissionKey,
): Promise<ConstituentOrgContext> {

...
interface RateLimitService {
  ...
  incrementWithExpiry(key: string, ttl: number): Promise<{ count: number; ttl: number }>;
  tokenBucketRefill(
    key: string,
    capacity: number,
    refillRate: number,
...
class InMemoryRateLimitService implements RateLimitService {
  ...
  async tokenBucketConsume(
    ...
    const state = await this.tokenBucketRefill(key, capacity, refillRate, refillInterval);
    
    if (state.tokens >= 1) {
      state.tokens -= 1;
      const resetTime = Math.floor(Date.now() / 1000) + Math.ceil(capacity / refillRate);
      return {
        allowed: true,
...
export { TokenBucketState, RateLimitService };
export default {
  increment,
  incrementWithExpiry,
  tokenBucketRefill,

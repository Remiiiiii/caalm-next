	...
	try {
		...
		if (entry.metadata) {
			...
			if (metadataString.length > 1000) {
				...
				if (entry.metadata.contractId)
					essentialMetadata.contractId = entry.metadata.contractId;
				if (entry.metadata.notificationId)
					essentialMetadata.notificationId = entry.metadata.notificationId;
				if (entry.metadata.eventId)
					essentialMetadata.eventId = entry.metadata.eventId;

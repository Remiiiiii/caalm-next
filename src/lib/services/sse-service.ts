...
class SSEService {
  ...
  private statusListeners: Map<string, Set<(status: SSEConnectionStatus) => void>> = new Map();
  ...
  subscribe(url: string, options: SSEOptions = {}): SSESubscription {
    ...
    eventSource.addEventListener('notification', (event: MessageEvent) => {
      try {
        ...
        console.error(`[SSEService] Error parsing notification from ${url}:`, error);
      }
    });
   ...
   */
  unsubscribeAll(): void {
    this.subscriptions.forEach((eventSource, url) => {
      this.unsubscribe(url);
    });
  }
    ...
    if (listeners) {
      listeners.forEach((listener) => {
        try {
          ...
          console.error(`[SSEService] Error in status listener for ${url}:`, error);
        }
      });
    }
  }

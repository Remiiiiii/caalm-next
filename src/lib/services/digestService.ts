...
import { NotificationDigestQueue } from '@/lib/database/schemas/notification-digest-queue.schema';
    ...
    try {
      ...
      for (const [userId, items] of userGroups) {
        try {
          ...
          const notifications = await this.getUserPendingDigestNotifications(
            userId
          );

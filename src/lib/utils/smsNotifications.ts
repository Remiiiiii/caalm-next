...
export async function notifyInvitationSent(
	...
	const adminAndExecWithPhones =
		await getUsersWithPhoneNumbers(adminAndExecUsers);
	const deptManagersWithPhones =
		await getUsersWithPhoneNumbers(deptManagerUsers);

...
async function verifyCollection() {
  try {
    ...
    const collectionId = appwriteConfig.contractDraftsCollectionId || '692f4a86002ae8f45cae';
    ...
    try {
      ...
      console.log('Collection attributes:', attributes.attributes.map((a: any) => a.key));
    } catch (error: any) {
      if (error.code === 404) {
        console.log('Collection does not exist. Please create it in Appwrite console with:');

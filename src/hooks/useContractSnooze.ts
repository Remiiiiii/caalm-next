		...
		async ({ contractId, days, expiryDate }: SnoozeContractParams) => {
			try {
				...
				if (contractId.startsWith("test-")) {
					throw new Error(
						"Cannot snooze test contracts. Please use a real contract from the database.",
					);
				}
				...
				if (!response.ok) {
					...
					const errorMessage =
						errorData.error || errorData.message || "Failed to snooze contract";
				...
				const errorMessage =
					error instanceof Error
						? error.message
						: "Failed to snooze contract. Please try again.";
				toast({
					title: "Error",

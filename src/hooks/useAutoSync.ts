...
export function useAutoSync(
	...
	useEffect(() => {
		...
		const performAutoSync = async () => {
			...
			const errorBackoffDelay =
				consecutiveErrorsRef.current > 0
					? Math.min(
							60000 * 2 ** (consecutiveErrorsRef.current - 1),
							5 * 60 * 1000,
			...
			if (timeSinceLastSync < errorBackoffDelay) {
				console.log(
					`⏳ Skipping auto-sync (too soon since last sync, backoff: ${errorBackoffDelay}ms)`,
				);
				return;
			}
			...
			try {
				...
				if (result.success) {
					...
					if (
						...
						result.error === "graph_connection_failed"
					) {
						if (consecutiveErrorsRef.current >= 3) {
							...
							toast({
								...
								description:
									"Please reconnect your Microsoft account in Settings to continue syncing.",
								variant: "destructive",
								duration: 8000,
							});
							...
							console.warn(
								`⚠️ Auto-sync failed (attempt ${consecutiveErrorsRef.current}/3):`,
								result.error,
							);
						}
					} else {
		...
		if (hasInitialSyncedRef.current) {
			console.log(
				"⏭️ Skipping initial sync (already performed in this session)",
			);
		...
		console.log(
			"✅ Auto-sync enabled (initial in 5s, then 30-second interval)",
		);

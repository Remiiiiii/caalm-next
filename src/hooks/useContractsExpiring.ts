...
const fetcher = async (url: string): Promise<UIFileDoc[]> => {
  try {
    ...
    console.log('[useContractsExpiring] Raw API response:', {
      ...
      dataLength: Array.isArray(rawResult.data) ? rawResult.data.length : 'not array',
    ...
    } else if (result.data && typeof result.data === 'object' && 'rows' in result.data) {
      // Appwrite format: { data: { rows: [...] } }
      contracts = (result.data as any).rows || [];
    } else {
      console.warn('[useContractsExpiring] Unexpected response format, returning empty array:', {
...
export function useContractsExpiring() {
  ...
  if (Array.isArray(data)) {
    ...
    console.warn('[useContractsExpiring] Data was wrapped in object, extracted array:', contracts.length);

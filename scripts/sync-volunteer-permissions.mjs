#!/usr/bin/env node
/**
 * Upsert volunteers.* permission rows and assign them to Super Admin / Org Admin.
 *
 * Usage:
 *   node scripts/sync-volunteer-permissions.mjs
 *   node scripts/sync-volunteer-permissions.mjs --demo
 */
...
const VOLUNTEER_PERMISSIONS = [
	...
	{
		$id: "perm_volunteers_manage",
		key: "volunteers.manage",
...
async function syncDatabase(databaseId) {
	console.log(`\nSync volunteer permissions in ${databaseId}`);
	for (const permission of VOLUNTEER_PERMISSIONS) {
...
console.log("\nVolunteer permissions sync complete.");

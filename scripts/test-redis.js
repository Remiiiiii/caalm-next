...
async function testRedis() {
  ...
  try {
    ...
    if (data.success) {
      ...
      console.log(`   Status: ${data.cache.healthy ? '✅ Healthy' : '❌ Unhealthy'}`);
      ...
      if (data.cache.healthy) {
        ...
        console.log('⚠️  Redis is configured but not healthy. Check the error above.');
        process.exit(1);
      }
    } else {
    ...
    console.log('   Make sure your development server is running: pnpm run dev');

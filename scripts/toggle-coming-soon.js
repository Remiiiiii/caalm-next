...
const _envExamplePath = path.join(process.cwd(), ".env.example");
...
function showStatus() {
	if (fs.existsSync(envPath)) {
		...
		if (match) {
			...
			console.log(
				"📊 Current status: SHOW_COMING_SOON not set (defaults to disabled)",
			);
		}
	} else {
		console.log(
			"📊 Current status: .env.local not found (defaults to disabled)",
		);
	}
}
...
switch (action) {
	...
	default:
		console.log(
			"Usage: node scripts/toggle-coming-soon.js [enable|disable|status]",
		);
		console.log("");
		...
		console.log("");
		console.log(
			"After running enable/disable, deploy to Vercel to apply changes.",
		);
		process.exit(1);
}

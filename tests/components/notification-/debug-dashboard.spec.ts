...
test.describe("Debug Dashboard Elements", () => {
	...
	test("inspect dashboard structure", async ({ page }) => {
		...
		await page.waitForSelector('main, [data-testid="dashboard"], body', {
			timeout: 5000,
		});
		...
		const elements = await page.evaluate(() => {
			const allElements = document.querySelectorAll(
				"[data-testid], [role], button, a, h1, h2, h3, nav, main, header, footer",
			);
		...
		console.log(
			"Dashboard debug completed - check test-results/dashboard-debug.png for visual reference",
		);
	});
});

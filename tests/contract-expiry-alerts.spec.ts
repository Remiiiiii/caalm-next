...
const mockContracts = [
  {
    ...
    contractExpiryDate: new Date(Date.now() + 5 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 5 days from now
    isExpired: false,
    daysUntilExpiry: 5,
    status: 'active',
  },
  {
    ...
    contractExpiryDate: new Date(Date.now() + 15 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 15 days from now
    isExpired: false,
    daysUntilExpiry: 15,
    status: 'active',
  },
  {
    ...
    contractExpiryDate: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 10 days ago
    isExpired: true,
    daysUntilExpiry: -10,
    status: 'expired',
  },
  {
    ...
    contractExpiryDate: new Date(Date.now() + 60 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 60 days from now
    isExpired: false,
    daysUntilExpiry: 60,
    status: 'active',
  },
...
test.describe('Contract Expiry Alerts Widget', () => {
  ...
  test('should display filter dropdown', async ({ page }) => {
    ...
    await page.waitForSelector('text=Contract Expiry Alerts', { timeout: 10000 });

    // Find the filter dropdown
    const filterSelect = page.locator('select[aria-label="Filter contracts by time period"]').first();
    await expect(filterSelect).toBeVisible();

    // Check that filter options are present
    await expect(filterSelect.locator('option[value="30"]')).toContainText('30 days');
    await expect(filterSelect.locator('option[value="60"]')).toContainText('60 days');
    await expect(filterSelect.locator('option[value="-1"]')).toContainText('Expired');
  });
  ...
  test('should show expired contracts when Expired filter is selected', async ({ page }) => {
    await page.waitForSelector('text=Contract Expiry Alerts', { timeout: 10000 });
  ...
  test('should show pulsating animation for expiring contracts badge', async ({ page }) => {
    await page.waitForSelector('text=Contract Expiry Alerts', { timeout: 10000 });
  ...
  test('should show empty state when no contracts match filter', async ({ page }) => {
    await page.waitForSelector('text=Contract Expiry Alerts', { timeout: 10000 });
  ...
  test('should handle error state', async ({ page }) => {
    ...
    const errorMessage = page.locator('text=Failed to load contract data').first();

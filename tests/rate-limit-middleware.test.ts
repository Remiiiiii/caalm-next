...
describe('Rate Limit Middleware', () => {
  ...
  it('should apply rate limiting to API routes', async () => {
    const { extractRateLimitIdentifier } = await import('@/lib/api/rate-limit/identifier.util');
  ...
  it('should bypass health check endpoints', async () => {
    const { shouldBypassRateLimit } = await import('@/lib/api/rate-limit/identifier.util');
    
    (shouldBypassRateLimit as any).mockReturnValue(true);

    const request = new NextRequest(new URL('http://localhost:3000/api/cache/health'));
    const response = await middleware(request);
